define([], function() {
  return {
      "PropertyPaneDescription": "Description",
      "BasicGroupName": "Group Name",
      "DescriptionFieldLabel": "Description Field",
      "CustomQueryModifier": {
          "GroupName":"Advanced Search Settings",
          "PrefixLabel":"Prefix Text",
          "PrefixDescription":"A string to prepend to each word found in the input query text.",
          "PrefixPlaceholder":"*",
          "SuffixLabel":"Suffix Text",
          "SuffixDescription":"A string to append to each word found in the input query text.",
          "SuffixPlaceholder":"*"
      },
      "MyOpportunitiesQueryModifier": {
        "GroupName": "Owner Opportunities Settings"
      },
      "classificationLevel": "Classification",
      "opportunityType": "Opportunity type",
      "duration": "Duration",
      "description": "Description",
      "location": "Location",
      "deadline": "Deadline",
      "view": "View",
      "apply": "Apply",
      "results": "results",
      "resultsFor": "results for",
      "viewAria": "View ",
      "applyAria": "Apply to ",
      "jobExpired": "This opportunity has expired.",
      "opportunities": "Opportunities",
      "applyExpired": "Applications closed",
      "undetermined": "Undetermined",
      "searchTermFound": "Search terms found in full post",
      "articlesTitle": "Latest news and articles",
      "articlesSearchTitle": "Browse news and articles",
      "views": "Views",
      "remote": "Remote"
  }
});